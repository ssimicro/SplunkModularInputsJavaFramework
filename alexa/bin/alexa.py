...
JAVA_MAIN_CLASS = 'com.splunk.modinput.alexa.AlexaWebService'
MODINPUT_NAME = 'alexa'
...
def run_java():
    ...
    if sys.platform.startswith('linux') or sys.platform.startswith('sunos') or sys.platform.startswith('aix') or sys.platform.startswith('hp-ux') or sys.platform.startswith('freebsd')  or sys.platform == 'darwin':
      ...
      CLASSPATH = MODINPUT_HOME + "bin/lib/*"+ ":" + MODINPUT_HOME + "dynamic_actions/lib"
      ...
      SPLUNK_HOME = os.path.expandvars('%SPLUNK_HOME%')
      MODINPUT_HOME = SPLUNK_HOME  + "\\etc\\apps\\"+MODINPUT_NAME+"\\"
      CLASSPATH = build_windows_classpath()+ ";" + MODINPUT_HOME + "dynamic_actions\\lib"
    else:
